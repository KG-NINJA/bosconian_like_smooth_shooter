...
function update() {
  ...
  if (player.x > MAP_W) player.x = 0;
  if (player.y < 0) player.y = MAP_H;
  if (player.y > MAP_H) player.y = 0;
...
function draw() {
  ...
  // マップラッピング用: 描画位置を全方向にずらして描画するヘルパー
  function wrapDraw(drawFunc) {
    for(let dx=-1; dx<=1; dx++) {
      for(let dy=-1; dy<=1; dy++) {
        ctx.save();
        ctx.translate(dx*MAP_W, dy*MAP_H);
        drawFunc();
        ctx.restore();
      }
    }
